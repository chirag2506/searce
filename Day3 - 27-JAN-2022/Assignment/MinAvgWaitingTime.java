...
class MinAvgWaitingTime {
    private static long computeMinAvg(NavigableMap<Long, List<Customer>> ordersByTime) {
        ...
        while (!ordersByTime.isEmpty() || !waitingCustomers.isEmpty()) {
            if (waitingCustomers.isEmpty()) {
                ...
                arrivals.values().stream().forEach(waitingCustomers::addAll);
                arrivals.clear();
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ordersByTime.computeIfAbsent(customer.arrivalTime, a -> new ArrayList<>()).add(customer);
        }
